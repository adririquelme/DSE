...
% with diagonal bandwidth matrix. The kernel is assumed to be Gaussian. The
% two bandwidth parameters are chosen optimally without ever using/assuming
% a parametric model for the data or any "rules of thumb". Unlike many
% other procedures, this one is immune to accuracy failures in the
% estimation of multimodal densities with widely separated modes (see
% examples). INPUTS: data - an N by 2 array with continuous data
%            n - size of the n by n grid over which the density is computed
%                n has to be a power of 2, otherwise n=2^ceil(log2(n)); the
%                default value is 2^8;
% MIN_XY,MAX_XY- limits of the bounding box over which the density is
% computed;
%                the format is: MIN_XY=[lower_Xlim,lower_Ylim]
%                MAX_XY=[upper_Xlim,upper_Ylim]. The dafault limits are
%                computed as: MAX=max(data,[],1); MIN=min(data,[],1);
%                Range=MAX-MIN; MAX_XY=MAX+Range/4; MIN_XY=MIN-Range/4;
% OUTPUT: bandwidth - a row vector with the two optimal
%                     bandwidths for a bivaroate Gaussian kernel; the
%                     format is: bandwidth=[bandwidth_X, bandwidth_Y];
%          density  - an n by n matrix containing the density values over
%          the n by n grid;
%                     density is not computed unless the function is asked
%                     for such an output;
%              X,Y  - the meshgrid over which the variable "density" has
%              been computed;
%                     the intended usage is as follows: surf(X,Y,density)
...
%         L_2 distance between the estimate and the true density) or you
%         have
