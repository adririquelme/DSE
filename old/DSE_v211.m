function varargout = DSE_v211(varargin)
...
% DSE_v211 MATLAB code for DSE_v211.fig
%      DSE_v211, by itself, creates a new DSE_v211 or raises the existing
%      singleton*.
%
%      H = DSE_v211 returns the handle to a new DSE_v211 or the handle to
%      the existing singleton*.
%
%      DSE_v211('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in DSE_V06.M with the given input arguments.
%
%      DSE_v211('Property','Value',...) creates a new DSE_V06 or raises the
...
% Edit the above text to modify the response to help DSE_v211

% Last Modified by GUIDE v2.5 15-Apr-2021 18:58:07
...
% Choose default command line output for DSE_v211
...
N = N+1;
handles.listbox_log.ListboxTop= N; % vista de la última línea
handles.data.log = S;
guidata(hObject,handles)
...
nsec=str2double(handles.box_nsec.String); % sectorización de la malla para el kde
angulovpples=str2double(handles.box_angulovpples.String); % mínimo ángulo entre polos principales
maxpples=str2double(handles.box_maxpples.String); % máximo número de polos principales
% guardo los datos con los que calculamos
...
% I es el índice de los puntos con cluster asignado
...
% Calculo el ángulo alpha
% alpha es el ángulo desde el OX antihorario hasta la proyección del polo
...
if uso==1
    if str2double(getfield(version,{find(contains(toolboxes,'Computer Vision Toolbox')==1)},'Version'))>=6.2
        ...
        % Genero los colores según la familia
...
ppcluster=4; % Es el número mínimo de puntos que tienen que tener un cluster para ser considerado como tal
...
handles.data.polos_estereoppalasignados;
[ puntos_familia_cluster_fullclusters, familia_cluster_plano_fullclusters, polos_estereoppalasignados_afterdbscan ] = f_ppal2cluster_v10( puntos_ppalasignados, planos_pples, ppcluster,vnfamilia, handles.data.polos_estereoppalasignados);
...
handles.data.polos_estereoppalasignados_afterdbscan=polos_estereoppalasignados_afterdbscan;
...
try
    ...
    % Por si acaso, guardo las columnas de la 4 al final
    [~,ncolumnas]=size(P);
    % Guardo las normales que están en las tres últimas columnas
    [npuntos,columnas]=size(P);
    planos=P(:,columnas-2:columnas);
    % compruebo que los planos están bien calculados
...
% Ahora preparo el análisis estadístico
tic
...
if isfield(handles.data,'log')
    ...
    handles.listbox_log.String=S;
    % fijo la vista del listbox en la última entrada
    handles.listbox_log.ListboxTop= 1; % vista de la última línea
    handles.data.log = S;
end
...
if popup==0
    ...
    if uso==1
        if str2double(getfield(version,{find(contains(toolboxes,'Computer Vision Toolbox')==1)},'Version'))>=6.2
            ...
            h.Color=[1 1 1];
            axis('square','equal');
            % Nota que I son los Ã­ndices de los puntos con clusters asignados,
...
if popup==0
    ...
    datacursormode on;
    % Aquí puede haber problema con el app nuevo
...
toolboxes={version.Name};
uso=length(find(contains(toolboxes,'Computer Vision Toolbox')==1));
% En realidad, pccnormals está siempre por lo que se puede ejecutar independientemente de su versión
tiempoempleado=floor(toc);
if uso==1
    ...
    % usamos la función nuestra
    [idx, dist, ~, calidad_tin, planos]=f_preparaTIN_v03_nocoplanartest(P, npb);
    handles.data.idx = idx; % En realidad, esto no se usa después
    handles.data.dist = dist; % Tampoco se usa después
...
try
    ...
    % Guardo únicamente el XYZ_Js, el RGB no sirve para nada en CC
    XYZ_JS=puntos_ppalasignados(:,1:3);
    XYZ_JS(:,4)=puntos_ppalasignados(:,4); % le pongo también las familias para que el escalar las pueda representar
...
try
    ...
    [ ~ ] = f_report_v02( pathname, filename, npoints, knn, tolerancia, nsec, angulovpples, cone, vnfamilia, ksigmas, nfamilias, handles.data.planos_pples, handles.data.familia_cluster_plano);
...
%% Changelog
% DSE 2.11
% 210415 Corrijo un error en cálculo de normales sin test ni toolboxes, que
% estaba en tiempoempleado
% Varias correcciones más debido a la migración a la versión 3, testeado
% para UVigo curso abril 2021
% DSE 2.10
% 20200126 Introduzco el análisis DBSCAN del toolbox de MATLAB, que es
% muuucho más rápido
% 20200120 Introduzco el cálculo de normales con el toolbox de MATLAB
% 20200213 Actualizo la programación para quitar el set y el get y migrar la app
% - cambio el dlmwrite por el writematrix que está disponible desde la 2019a
