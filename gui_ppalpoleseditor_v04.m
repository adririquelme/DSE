function varargout = gui_ppalpoleseditor_v04(varargin)
...
% GUI_PPALPOLESEDITOR_V04 MATLAB code for gui_ppalpoleseditor_v04.fig
%      GUI_PPALPOLESEDITOR_V04, by itself, creates a new GUI_PPALPOLESEDITOR_V04 or raises the existing
%      singleton*.
%
%      H = GUI_PPALPOLESEDITOR_V04 returns the handle to a new GUI_PPALPOLESEDITOR_V04 or the handle to
%      the existing singleton*.
%
%      GUI_PPALPOLESEDITOR_V04('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in GUI_PPALPOLESEDITOR_V04.M with the given input arguments.
%
%      GUI_PPALPOLESEDITOR_V04('Property','Value',...) creates a new GUI_PPALPOLESEDITOR_V04 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before gui_ppalpoleseditor_v04_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to gui_ppalpoleseditor_v04_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help gui_ppalpoleseditor_v04

% Last Modified by GUIDE v2.5 10-Aug-2015 17:42:50

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @gui_ppalpoleseditor_v04_OpeningFcn, ...
                   'gui_OutputFcn',  @gui_ppalpoleseditor_v04_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before gui_ppalpoleseditor_v04 is made visible.
function gui_ppalpoleseditor_v04_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to gui_ppalpoleseditor_v04 (see VARARGIN)

% Choose default command line output for gui_ppalpoleseditor_v04
...
% polos_estereo_cartesianas=getappdata(0,'polos_estereo_cartesianas');
% ponemos los polos en la tabla4
set(handles.uitable_ppalpoles,'Enable','on','Data',getappdata(0,'global_ppal_poles'));
% guardamos las variables en las handles
handles.polos_estereo_cartesianas=getappdata(0,'polos_estereo_cartesianas');
handles.ppal_poles=getappdata(0,'global_ppal_poles');
handles.polos_pples_cart=getappdata(0,'global_polos_pples_cart');
handles.X=getappdata(0,'global_X');
handles.Y=getappdata(0,'global_Y');
handles.density=getappdata(0,'global_density');
...
% UIWAIT makes gui_ppalpoleseditor_v04 wait for user response (see UIRESUME)
% uiwait(handles.figure1);



% --- Outputs from this function are returned to the command line.
function varargout = gui_ppalpoleseditor_v04_OutputFcn(hObject, eventdata, handles) 
...
% configuro el entorno para la nueva dimensión
set(handles.uitable_ppalpoles,'Enable','on','Data',ppal_poles);
% compruebo que el polo seleccionado no excede la dimensión
[n,~]=size(ppal_poles);
if nppalpole>n
    set(handles.box_pole2remove,'String',num2str(n));
end
% guardo la variable en las handles
handles.ppal_poles = ppal_poles;
...
yq=0;
zq=griddata(X,Y,density,xq,yq);
% inicio el vector del nuevo polo. Cuidado, le añado la cuarta componente
% correspondiente al porcentaje de puntos del nuevo polo respecto al total.
...
% Función que representa la función de densidad con los nuevos polos
% principales
polos_estereo_cartesianas = handles.polos_estereo_cartesianas;
...
% la tabla
cone = str2double(get(handles.box_cone,'String'))/180*pi;
% Asignamos cada punto a su polo principal
[polos_estereoppalasignados, ~] = f_setppal2planes( cone, polos_estereo_cartesianas, polos_pples_cart ,P);
[n,~]=size(P); % número de puntos totales
[nf,~]=size(ppal_poles); % número de familias que hay
for i=1:nf
    ...
    ppal_poles(i,4)=npuntos/n*100;
end
% actualizo la tabla de salida
set(handles.uitable_ppalpoles,'Enable','on','Data',ppal_poles);
handles.ppal_poles=ppal_poles;
setappdata(0,'global_ppal_poles',ppal_poles);
